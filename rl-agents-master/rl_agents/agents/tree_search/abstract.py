...
class AbstractTreeSearchAgent(AbstractAgent):
    ...
    def write_tree(self):
        if self.config["display_tree"] and hasattr(self, 'writer') and self.writer:
            try:
                TreePlot(self.planner, max_depth=4).plot_to_writer(self.writer, epoch=self.steps, show=False, figsize=(12, 8))
            except Exception as e:
                print(f"[yellow]⚠️ 树可视化失败: {e}[/yellow]")
