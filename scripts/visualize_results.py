...
# Use GPT configuration (only supported provider now)
llm_config = config['gpt_config']

# Set OpenAI API environment variables
os.environ["OPENAI_API_TYPE"] = 'openai'
os.environ["OPENAI_API_KEY"] = llm_config['OPENAI_KEY']
os.environ["OPENAI_CHAT_MODEL"] = llm_config['OPENAI_CHAT_MODEL']
os.environ["OPENAI_API_BASE"] = llm_config['OPENAI_API_BASE']
...
def viewFrame(decisionFrame: int) -> str:
    # 1-indexed display -> internal 0-indexed
    actualFrame = minFrame + int(decisionFrame) - 1
    imd = esr.plotSce(actualFrame)
    framePrompts = esr.getPrompts(actualFrame)
...
def commitExperience(decisionFrame: int, expertExperience: str):
    try:
        # 1-indexed display -> internal 0-indexed
        internal_index = minFrame + int(decisionFrame) - 1
        framePrompts = esr.getPrompts(internal_index)
        ...
        esr.editTA(internal_index, expertExperience)
...
if __name__ == '__main__':
    ...
    # 1-indexed display range, ensure first frame is not excluded
    display_min = 1
    display_max = maxFrame - minFrame + 1
    ...
    with gr.Blocks(theme=gr.themes.Base(text_size=gr.themes.sizes.text_lg)) as demo:
        with gr.Row(visible=True, variant='panel'):
            ...
            frameRange = range(display_min, display_max + 1)
            decisionFrame = gr.Dropdown(
                frameRange, value='1', label="Decision Frame"
            )
